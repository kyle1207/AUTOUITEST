...
class TestAddUserDevGroupMgmt:
    ...
    def user_dev_login(self):
        ...
        yield
        self.user_dev_logout()

    def user_dev_logout(self):
    ...
    @pytest.mark.usefixtures("user_dev_login")
    def test_user_role_judge_dev(self):
        self.user_dev_judge_vstudio()

    @pytest.mark.run(order=3)
    @pytest.mark.usefixtures("admin_delete_login")
    def test_delete_user_group_dev(self):
        self.delete_user_group_dev()

    def delete_user_group_dev(self):
        ...
        self.pageadminMgmt.delete_group(groupNameList[2][0])
    ...
    def user_group_assign(self):
        self.logger.info("普通用户")
        adduser = GetYaml("user_Mgmt.yaml").find_userMgmt()
        groupNameList = GetYaml("user_Mgmt.yaml").get_groupNames_Mgmt()
        self.assign_usergroup(adduser, groupNameList[2])
