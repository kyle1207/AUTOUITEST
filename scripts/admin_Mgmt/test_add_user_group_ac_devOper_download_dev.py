...
class TestAddUserAcOpDownDevGroupMgmt:
    ...
    # 创建用户选择数采运营测试组 + 数据下载测试组 + 普通用户测试组 + 开发运营测试组
    ...
    def test_add_user_group_ac_download(self, username, email, password, surname, name):
        self.logger.info("创建用户选择数采运营测试组+数据下载测试组+普通用户测试组+开发运营测试组")
    ...
    # 创建的用户登录数采应用平台：数采运营测试组 + 数据下载测试组菜单权限是否正常
    ...
    # 创建的用户登录开发运营平台：普通用户测试组 + 开发运营测试组菜单权限是否正常
    ...
    def user_group_assign(self):
        self.logger.info("数采运营+开发运营+普通用户+数据下载")
        adduser = GetYaml("user_Mgmt.yaml").find_userMgmt()
        groupNameList = GetYaml("user_Mgmt.yaml").get_groupNames_Mgmt()
        self.assign_usergroup(adduser, groupNameList[5])
