...
class TestAddUserAcDownGroupMgmt:
    ...
    def test_add_user_group_ac_download(self, username, email, password, surname, name):
        self.logger.info("创建用户选择数采运营测试组+数据下载测试组")
    ...
    # 判断数采运营测试组+数据下载测试组菜单权限是否正常
    @pytest.mark.run(order=2)
    @pytest.mark.usefixtures("user_login")
    def test_user_role_judge(self):
        self.logger.info("判断数采运营测试组+数据下载测试组菜单权限是否正常")
        self.user_role_judge()

    # 删除用户、采运营测试组、数据下载测试组
    @pytest.mark.run(order=3)
    @pytest.mark.usefixtures("admin_login")
    def test_delete_user_group_ac_download(self):
        self.logger.info("删除用户、采运营测试组、数据下载测试组")
        self.delete_user_group_ac_download()

    def delete_user_group_ac_download(self):
        ...
        self.pageadminMgmt.delete_group(groupNameList[7][0])
        self.pageadminMgmt.delete_group(groupNameList[7][1])
    ...
    def user_group_assign(self):
        self.logger.info("数采运营+数据下载")
        adduser = GetYaml("user_Mgmt.yaml").find_userMgmt()
        groupNameList = GetYaml("user_Mgmt.yaml").get_groupNames_Mgmt()
        self.assign_usergroup(adduser, groupNameList[7])
