...
class TestAddUserOpGroupMgmt:
    ...
    def user_devOper_logout(self):
        element = self.role_pageroleMgmt.element_find(role_Mgmt.roleMgmt_devOper_list)
    ...
    def delete_user_group_devOper(self):
        ...
        self.pageadminMgmt.delete_group(groupNameList[6][0])
    ...
    def user_group_assign(self):
        self.logger.info("开发运营")
        adduser = GetYaml("user_Mgmt.yaml").find_userMgmt()
        groupNameList = GetYaml("user_Mgmt.yaml").get_groupNames_Mgmt()
        self.assign_usergroup(adduser, groupNameList[6])
